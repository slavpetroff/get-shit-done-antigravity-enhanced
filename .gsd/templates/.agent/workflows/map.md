...
- Library Intelligence Findings (from `scripts/recall_context.py` in Step 2)
- Discovered Skills (from `SKILLS.md`)
- Codebase structure analysis (Step 2 - 5)

**Instruction**:
When writing `ARCHITECTURE.md`, you MUST cross-reference your codebase findings with the **Library Intelligence**. For example:

- If `FastAPI` is detected, use the prescribed directory patterns from library context.
- If anti-patterns are identified in library context, flag them in the Technical Debt section.
- Incorporate best practices from discovered Skills (e.g., `codebase-mapper`).

```markdown
# Architecture

> Auto-generated by /map on {date}
> Injected Intelligence: [Yes]
